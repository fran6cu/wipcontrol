<template>
  <section>
    ...
    <ModalConfirmation v-if="showModal" @close="hideConfirmation" @delete="deleteTaskComment"
      :msg="'Are you sure?'"></ModalConfirmation>
    <FormModal v-if="showAddCommentTask" @close="hideForm" :formType="'addCommentTask'">
    </FormModal>
  </section>
</template>
 ...
 * @module Component/StoryDetails/CommentTask
 * 
 ...
 * @property {boolean} showAddCommentTask - show id comment
...
export default {
  ...
  methods: {
    ...
    async getTaskComments() {
      ...
      return await fetch(`${apiUrl}/taskComments/task/${this.$route.params.id}/page/0`)
    ...
    deleteTaskComment() {
      ...
      fetch(`${apiUrl}/taskComments/${this.idCommentToDel}`, options)
        .then(response => response.json())
        .then(data => {
          if (data.id) {
     ...
     */
    showForm() {
      this.showAddCommentTask = true;
    },
    /**
     * hide form add comment
     */
    hideForm() {
      this.showAddCommentTask = false;
    }
  },
  mounted() {
    this.isLoading = true;
